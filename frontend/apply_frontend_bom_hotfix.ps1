# apply_frontend_bom_hotfix.ps1
# Run from your FRONTEND project root (e.g. C:\Users\ABC\Desktop\ai_tutor_app\frontend)
# HOTFIX: re-writes quiz_service.dart and live_class_room_screen.dart with the stray
# UTF-8 BOM removed - flutter analyze caught this as a real compile error
# ('Illegal character 239/187/191') introduced when those files' pre-existing BOM
# got mangled passing through a non-BOM PowerShell script under Windows PowerShell 5.1.
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force

$root = Get-Location
Write-Host "Applying BOM hotfix in $root" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Done. Run: flutter analyze  (both BOM errors should be gone now)" -ForegroundColor Yellow